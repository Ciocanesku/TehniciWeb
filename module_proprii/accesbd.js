...
class AccesBD{
    ...
    initLocal(){          
        this.client= new Client({database:"proiect_web", 
            user:"gioni", 
            password:"123456", 
            host:"localhost", 
            port:5432});              //utilizaror baza de date
        this.client.connect();
    }
     ...
     * @returns {AccesBD}
     */
    static getInstanta({init="local"}={}){ //AccesBD.getInstanta({init:"render"})
    ...
    async selectAsync({tabel="",campuri=[],conditiiAnd=[]} = {}){
        let conditieWhere="";
        if(conditiiAnd.length>0)
            conditieWhere=`where ${conditiiAnd.join(" and ")}`; //["cod=17","nume"="ion"] where cod=17 and nume=ion
    ...
    insert({tabel="",campuri={}} = {}, callback){  //AcessBD.getInstanta().insert({tabel:"produse",campuri:{nume:"savarina"
                                                  //                                                        pret:"10"}})
        console.log("-------------------------------------------")
